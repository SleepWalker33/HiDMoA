...
运行结果保存:
  - runs/<timestamp>/
  - 子目录 incre1 / full1 / full2
  - 单 seed：根目录 test.txt；每方法目录 train.txt + test.txt
  - 多 seed：每方法/seed_N 下 train.txt + test.txt；根目录 test_repeat.txt（跨 seed 均值±标准差）
"""
...
_REPORT_SECTION_PROFILE = {
    "incremental_1": "incre1",
    "incremental_2": "incre2",
    "incremental_3": "incre3",
    "full_1": "full1",
    "full_2": "full2",
}
...
def _parse_session_title(title: str) -> Optional[Tuple[str, int, str]]:
    ...
    Returns (kind, session_num, suffix):
      - ("session", N, "")       -> #sessionN (incre1 / PyCIL)
